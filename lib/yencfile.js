...
/**
 * Decode an article containing =ybegin headers
 *
 * @author   Jelle De Loecker   <jelle@kipdola.be>
 * @since    0.1.0
 * @version  0.1.0
 ...
 */
YencFile.prototype.decodeArticle = function decodeArticle(article) {
	...
	for (i = 0; i < lines.length; i++) {
		...
		if (!ybegin || ybegin && ypart == null) {

			// Skip junk lines at the start of the body
			if (!ybegin && lines[i].indexOf('=ybegin') == -1) {
				continue;
			}

			// If there is a ypart info line it should be right after ybegin
			if (ybegin && lines[i].indexOf('=ypart') == -1) {
				ypart = false;
				i--;
				break;
			}

			if (ybegin) ypart = true;
	...
	// Get the index of the stopping line (exclusive)
	if (yend == null) {
	...
	// Create the buffer where the bytes will go
	this.buffer = new Buffer(this.info.partsize || this.info.filesize);
